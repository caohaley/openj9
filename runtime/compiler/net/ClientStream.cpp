...
int openConnection(const std::string &address, uint32_t port, uint32_t timeoutMs)
   ...
   if (omrsock_socket(OMRPORTLIB, &socket, OMRSOCK_AF_INET, OMRSOCK_STREAM, OMRSOCK_IPPROTO_DEFAULT) < 0 || (socket == NULL))
   ...
   OMRLinger lingerVal;
   OMRPORTLIB->sock_linger_init(OMRPORTLIB, &lingerVal, 1, 2);
   if (omrsock_setsockopt_linger(socket, OMRSOCK_SOL_SOCKET, OMRSOCK_SO_LINGER, &lingerVal) < 0)
      {
      omrsock_close(&socket);
   ...
   OMRTimeval timeout;
   OMRPORTLIB->sock_timeval_init(OMRPORTLIB, &timeVal, timeoutMs / 1000, (timeoutMs % 1000) * 1000);
   ...
   if (OMRPORTLIB->sock_setsockopt_int(socket, OMRSOCK_IPPROTO_TCP, OMRSOCK_TCP_NODELAY, &flag) < 0)
      {
      omrsock_close(&socket);
   ...
   if (omrsock_getaddrinfo(address.c_str(), omrsock_htons(port), hints, &result))
   ...
   /* Try to connect to a server with results. */
	for (unsigned int i = 0; i < len; i++) {
      omrsock_addrinfo_address(OMRPORTLIB, &result, i, &servAddr);
